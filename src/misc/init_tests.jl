...
function check_bipartiteness(A, T, B)
  ...
  length(T) == size(A,2) || throw(ArgumentError("The length of top-level species names must match matrix size ($(length(T)) v. $(size(A,1)))"))
  length(B) == size(A,1) || throw(ArgumentError("The length of bottom-level species names must match matrix size ($(length(B)) v. $(size(A,2)))"))
end

function check_unipartiteness(A, S)
  check_unipartiteness(A)
  check_name_vector(S)
  length(S) == size(A,2) || throw(ArgumentError("The length of species names must match matrix size ($(length(S)) v. $(size(A,1)))"))
end
