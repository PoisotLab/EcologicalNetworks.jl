...
"""
mutable struct BlockModel{NT <: AbstractEcologicalNetwork,IT <: Integer, FT <: AbstractFloat} <: NetworkGenerator
    networktype::Type{NT}
    size::Tuple{IT,IT}
...
BlockModel(labels::Vector{T}, blocks::Matrix{FT}) where {T <: Integer, FT <:AbstractFloat} = begin
    nlabs = length(unique(labels))

    # check params
    if size(labels[1]) != size(labels[2]) || length(unique(labels[1])) != length(unique(labels[2]))
        @error "not a unipartite set of labels"
    end
    BlockModel(UnipartiteNetwork, (nlabs,nlabs), (labels,labels), (nlabs,nlabs), blocks);
end 


# biipartite constructor from a tuple of labels and a block matrix
BlockModel(labels::Tuple{Vector{T},Vector{T}}, blocks::Matrix{FT}) where {T <: Integer, FT <:AbstractFloat} = begin
    tlabs = length(unique(labels[1]))
    blabs = length(unique(labels[2]))
  
    BlockModel(BipartiteNetwork, (length(tlabs),length(blabs)), (labels[1], labels[2]), (tlabs,blabs), blocks);
end 


function _isunipartitegenerator(gen)
    size(gen.nodelabels[1]) == size(gen.nodelabels[2]) && length(unique(gen.nodelabels[1])) == length(unique(gen.nodelabels[2]))
end


function _isbipartitegenerator(gen)
    size(gen.blocks)[1] == length(unique(gen.nodelabels[1])) && size(gen.blocks)[2] == length(unique(gen.nodelabels[2]))
end
